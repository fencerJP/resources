...
impl GpuImpl for AmdGpu {
    ...
    fn name(&self) -> Result<String> {
        ...
        Ok((*AMDGPU_IDS)
            .get(&(self.device().map_or(0, Device::pid), revision))
            .cloned()
            .unwrap_or_else(|| {
                self.drm_name().map_or_else(
                    |_| "AMD Radeon Graphics".into(),
                    |drm_name| format!("AMD Radeon Graphics ({drm_name})"),
                )
            }))
    }
